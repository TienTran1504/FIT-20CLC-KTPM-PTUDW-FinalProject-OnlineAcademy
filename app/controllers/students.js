...
const getCourseList = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json({ id: user._id, name: user.name, courseList: user.courseList })
}
...
const addCourse = async (req, res) => {
    ...
    if (course === null) {
        throw new NotFoundError(`Dont have course with id: ${courseId}`)
    }
    else if (checkCourseExist) {
        throw new BadRequestError(`Already have course with id ${courseId} in student course's list`)
    }
    else {
        const item = { course };
        ...
        res.status(StatusCodes.OK).json({ msg: `Add ${course.name} successfully`, name: userUpdate.name, courseList: userUpdate.courseList, courseLength: userUpdate.courseList.length })
...
const removeCourse = async (req, res) => {
    ...
    if (!checkCourseExist) {
        throw new BadRequestError(`Dont exist course with id ${courseId} in student course's list`)
    }
    ...
    res.status(StatusCodes.OK).json({ msg: `Delete course: ${course.name} from student course's list successfully`, _id: userUpdate._id, name: userUpdate.name, courseList: userUpdate.courseList, courseLength: userUpdate.courseList.length })
}
...
const updateStatusCourse = async (req, res) => {
    ...
    if (!status) {
        throw new BadRequestError(`Status field can not be empty`)
    }
    else if (status !== "in progress" && status !== "completed") {
        throw new BadRequestError(`Status field is completed or in progress`)
    }
    else if (!checkCourseExist) {
        throw new BadRequestError(`Dont exist course with id ${courseId} in student course's list`)
    }
        ...
        res.status(StatusCodes.OK).json({ msg: `Update quantity ${courseId} successfully`, name: userUpdate.name, courseList: userUpdate.courseList, courseLength: userUpdate.courseList.length })
    }
}
